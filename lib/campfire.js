...
var Campfire = function(options) {
  ...
  if (options.oauth) {
    this.authorization = "Bearer " + (options.token || options.oauth);
  } else {
...
Campfire.prototype.room = function(id, callback) {
  var self = this;
  if (rooms[id]) id = rooms[id].id;
  if (typeof id == 'number') return roomByNumber();

  return self.rooms(function(error, response) {
    if (error) {
      return callback(error);
    }
    rooms = {};
    ...
    return callback(new Error('No such room: '+ id));
  });

  function roomByNumber() {
    self.get("/room/" + id, function(error, response) {
      if ((error = error || response.error)) {
        return callback(error);
      }
      if (response) {
        var room = new Room(self, response.room);
      }
